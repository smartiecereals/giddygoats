...
function getCrimeAreas (lat, long) {
  // fetch('../', {
  //   method: 'POST', 
  //   mode: 'cors', 
  //   headers: new Headers({
  ...
  // })
  // .catch(function(err) {
  //   console.log('Heat map fetch error', err);
  // })
  return [{
    lat: 37.782551,
    long: -122.445368,
...
function getPoints() {
  ...
  //getCrimeAreas into array
  var crimeAreas = getCrimeAreas()
  //create an array for google maps coordinates
  var mapPoints = [];
  //For each of the items in this array
  for (let i = 0; i < crimeAreas.length; i++) {
    for (let j = 0; j < crimeAreas[i].count; j++) {
    //Create a new google map coordinate for each of the counts
        mapPoints.push(new google.maps.LatLng(crimeAreas[i].lat,crimeAreas[i].long));
    }
  }
  // Return this final array
  return mapPoints
  // [
